...
[y2, fs_in2] = readwav([oracle_data.target_position_csv_path(1:find(oracle_data.target_position_csv_path=='.',1,'last')) 'wav'],'g');
if isempty(params.fs)
    params.fs = fs_in;
else
    y = resample(y,params.fs,fs_in);
    y2 = resample(y2,params.fs,fs_in2);
end
...
[Y_oracle,x_tail_anal,pm] = stft_v2('fwd', y-y2, ...
    stft_params.win_anal, ...
    stft_params.sig_frame_inc,...
    stft_params.nfft,...
    stft_params.fs);
...
fprintf('Computing MPDR covariance matrices...\n')
% precompute the instantaneous outer product
ryyH = bsxfun(@times,(permute(Y_oracle,[2 4 1 3])),conj(permute(Y_oracle,[4 2 1 3]))); %[nChan, nChan, nFreq,nFrames]

[~,nInitFrames]=min(abs(pm.t-params.noise_only_init_time));
rRyest = mean(ryyH(:,:,:,1:nInitFrames),4); %[nChan, nChan, nFreq]
%rRyest_offline = mean(ryyH,4); %[nChan, nChan, nFreq] 
...
for iframe = 1:nFrames
    ...
    % update smoothed sample covariance matrix
    rRyest = ax * rRyest + axc * ryyH(:,:,:,iframe);

    for ifreq = 1:nFreq
        
        R = rRyest(:,:,ifreq); % online 
        %R = rRyest_offline(:,:,ifreq); % offline
